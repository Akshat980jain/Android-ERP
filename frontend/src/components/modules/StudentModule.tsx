  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-200">Students</h2>
        {user?.role !== 'student' && (
          <button
      ...
      {(user?.role === 'faculty' || user?.role === 'admin') && (
        <div className="mb-4">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select Course to Add Students</label>
          <select
        ...
        {loading ? (
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {filteredStudents.map((student) => (
              <div key={student._id} className="flex items-center justify-between p-3 border rounded hover:bg-gray-50 dark:bg-gray-700/50">
                <div className="flex-1">
                  <div className="font-medium">{student.name}</div>
                  <div className="text-sm text-gray-600 dark:text-gray-400">{student.studentId} • {student.email}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">Department: {student.department}</div>
                  {/* Display enrolled courses */}
                  {getStudentEnrolledCourses(student._id).length > 0 && (
                    <div className="mt-2">
                      <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">Enrolled Courses:</div>
                      <div className="flex flex-wrap gap-1">
                        {getStudentEnrolledCourses(student._id).map(course => (
                          <span key={course._id} className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-400 px-2 py-1 rounded">
                            {course.name} ({course.code})
                          </span>
                        ))}
                      </div>
