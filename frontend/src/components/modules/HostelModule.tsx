  ...
  return (
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      <div className="flex items-center space-x-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Hostel Management</h1>
      </div>
      ...
      {loading ? (
        <div className="text-center py-8 text-gray-500 dark:text-gray-400">Loading hostels...</div>
      ) : (
            ...
            <Card key={h._id}>
              ...
              <CardContent>
                <div className="space-y-3">
                  <div className="text-sm text-gray-700 dark:text-gray-300 flex items-center"><Users className="w-4 h-4 mr-2" /> Rooms: {h.rooms.length}</div>
                  <div className="grid grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                    {h.rooms.map(r => (
                      <div key={r.number} className="border border-gray-300 dark:border-gray-600 rounded p-2 text-xs bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                  ...
                  {user?.role === 'admin' && (
                    <div className="border-t pt-3 mt-2">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
                        <select value={allocHostelId} onChange={e => setAllocHostelId(e.target.value)} className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                          <option value="">Hostel</option>
                          {hostels.map(x => <option key={x._id} value={x._id}>{x.name}-{x.block}</option>)}
                        </select>
                        <input value={allocRoom} onChange={e => setAllocRoom(e.target.value)} placeholder="Room No" className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
                        <input value={allocStudentId} onChange={e => setAllocStudentId(e.target.value)} placeholder="Student ID" className="border border-gray-300 dark:border-gray-600 rounded px-2 py-1 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
