    ...
    <ChartCard title={title} className={className}>
      <div className="h-64 p-6">
        <div className="flex justify-between items-center mb-4">
          {xAxis && <span className="text-sm text-gray-600 dark:text-gray-400">{xAxis}</span>}
          {yAxis && <span className="text-sm text-gray-600 dark:text-gray-400">{yAxis}</span>}
        </div>
        <div className="space-y-3">
          {data.map((item, index) => (
            <div key={index} className="flex items-center space-x-3">
              <div className="w-24 text-sm text-gray-600 dark:text-gray-400 truncate">{item.name}</div>
